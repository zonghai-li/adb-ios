...
adb_thread_t server_thread;
static void* _launch_server_routine(void *arg);
 ...
 * enable all tracing
 */
void  adb_trace_init(const char *spec)
{
    const char*  p = spec;
    const char*  q;
    ...
    while (*p) {
        ...
        {
            size_t taglen = strlen(tags[tagn].tag);
...
int launch_server(int server_port)
{
    ...
    // try running the server in a thread.
    {
        adb_thread_create(&server_thread, _launch_server_routine, NULL);
    }
